...
# also added columns from "Strepsirrhine reproductive traits.xlsx" - from a google doc with Leila
# and Janis looked up the WoS counts
traits_data = read.csv("trait_data_strepsirrhines_Jan22.csv", header=T)
...
########################################################
### Start exploring some of the non-mating behavior related traits that 
### could have to do with white blood cells 
...
#RBC ~ BM
pgls_bm = gls(mean_value_Red.Blood.Cells~mean_value_Body.Weight, data=traits_data, 
            correlation = corPagel(0.5, tree, form=~Species, fixed=FALSE),
            method="REML", weights=~I(1/n_ind_Red.Blood.Cells))
summary(pgls_bm)# *  p = 0.16 ##REALLY high lambda = 0.97
...
##############################################3
##*this part is new as of Jan 21: ##
##*
#### LONGEVITY
## none of the wbc types are significantly predicted by longevity (though there is pretty
## strong phylogenetic signal!)
## results for longevity do not qualitatively change if control for body mass

hist(traits_data$L_Median_All_Longevity_gt30d_y)
summary(traits_data$L_Median_All_Longevity_gt30d_y)
traits_data.lgv = subset(traits_data, !is.na(L_Median_All_Longevity_gt30d_y))

nameck = name.check(tree, traits_data.lgv, data.names=traits_data.lgv$Species)
nameck
tree.lgv=drop.tip(tree, nameck$tree_not_data)

#Total WBC ~ longevity
pgls_lgv = gls(mean_value_White.Blood.Cells*1000~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0.5, tree.lgv, form=~Species, fixed=FALSE), 
              method="REML", weights=~I(1/n_ind_White.Blood.Cells))
summary(pgls_lgv) #p=0.43

#neutrophils ~ longevity
pgls_lgv = gls(mean_value_Neutrophil.Seg.Abs/1000~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0.5, tree.lgv, form=~Species, fixed=FALSE), 
              method="REML", weights=~I(1/n_ind_Neutrophil.Seg.Abs))
summary(pgls_lgv) # p = 0.16

#lymphocytes ~ longevity
pgls_lgv = gls(mean_value_Lymphocytes.Abs~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0.5, tree.lgv, form=~Species, fixed=FALSE), 
              method="REML", weights=~I(1/n_ind_Lymphocytes.Abs))
summary(pgls_lgv) #p= 0.50

#basophils ~ longevity
pgls_lgv = gls(mean_value_Basophils.Abs~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0.5, tree.lgv, form=~Species, fixed=FALSE), 
              method="REML", weights=~I(1/n_ind_Basophils.Abs))
summary(pgls_lgv) #p= 0.77, but lambda is negative, so re-fit

pgls_lgv = gls(mean_value_Basophils.Abs~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0, tree.lgv, form=~Species, fixed=TRUE), 
              method="REML", weights=~I(1/n_ind_Basophils.Abs))
summary(pgls_lgv) #p= 0.93

#eosinophils ~ longevity
pgls_lgv = gls(mean_value_Eosinophils.Abs~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0.5, tree.lgv, form=~Species, fixed=FALSE), 
              method="REML", weights=~I(1/n_ind_Eosinophils.Abs))
summary(pgls_lgv) #p= 0.30

#monocytes ~ longevity
pgls_lgv = gls(mean_value_Monocytes.Abs~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0.5, tree.lgv, form=~Species, fixed=FALSE), 
              method="REML", weights=~I(1/n_ind_Monocytes.Abs))
summary(pgls_lgv) #p= 0.16

#RBC ~ longevity
pgls_lgv = gls(mean_value_Red.Blood.Cells~L_Median_All_Longevity_gt30d_y, data=traits_data.lgv, 
              correlation = corPagel(0.5, tree.lgv, form=~Species, fixed=FALSE),
              method="REML", weights=~I(1/n_ind_Red.Blood.Cells))
summary(pgls_lgv)# *  p = 0.16 ##REALLY high lambda = 0.97
...
hist(traits_data$Latitude_GMPD)
summary(traits_data$Latitude_GMPD)
traits_data.lat = subset(traits_data, !is.na(traits_data$Latitude_GMPD))

nameck = name.check(tree, traits_data.lat, data.names=traits_data.lat$Species)
nameck
tree.lat=drop.tip(tree, nameck$tree_not_data)

#Total WBC ~ latitude
pgls_lat = gls(mean_value_White.Blood.Cells*1000~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0.5, tree.lat, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_White.Blood.Cells))
summary(pgls_lat) #p=0.43

pgls_lat = gls(mean_value_White.Blood.Cells*1000~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0, tree.lat, form=~Species, fixed=TRUE), 
               method="REML", weights=~I(1/n_ind_White.Blood.Cells))
summary(pgls_lat) #p=0.86

#neutrophils ~ latitude
pgls_lat = gls(mean_value_Neutrophil.Seg.Abs/1000~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0.5, tree.lat, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Neutrophil.Seg.Abs))
summary(pgls_lat) # p = 0.71

#lymphocytes ~ latitude
pgls_lat = gls(mean_value_Lymphocytes.Abs~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0.5, tree.lat, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Lymphocytes.Abs))
summary(pgls_lat) #p= 0.97

#basophils ~ latitude * approaching significance ??? look for relationship with parasites?
pgls_lat = gls(mean_value_Basophils.Abs~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0.5, tree.lat, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Basophils.Abs))
summary(pgls_lat) #p= 0.053 * ??? lambda = 0.04

#eosinophils ~ latitude
pgls_lat = gls(mean_value_Eosinophils.Abs~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0.5, tree.lat, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Eosinophils.Abs))
summary(pgls_lat) #p= 0.97

#monocytes ~ latitude
pgls_lat = gls(mean_value_Monocytes.Abs~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0.5, tree.lat, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Monocytes.Abs))
summary(pgls_lat) #p= 0.26

#RBC ~ latitude
pgls_lat = gls(mean_value_Red.Blood.Cells~abs(Latitude_GMPD), data=traits_data.lat, 
               correlation = corPagel(0, tree.lat, form=~Species, fixed=TRUE),
               method="REML", weights=~I(1/n_ind_Red.Blood.Cells))
summary(pgls_lat)# *  p = 0.55 #fixed lambda at 0 because it was negative



### ELEVATION

hist(traits_data$Upper_elevation)
summary(traits_data$Upper_elevation)
traits_data.elev = subset(traits_data, !is.na(traits_data$Upper_elevation))

nameck = name.check(tree, traits_data.elev, data.names=traits_data.elev$Species)
nameck
tree.elev=drop.tip(tree, nameck$tree_not_data)

#Total WBC ~ elevation
pgls_elev = gls(mean_value_White.Blood.Cells*1000~Upper_elevation, data=traits_data.elev, 
               correlation = corPagel(0.5, tree.elev, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_White.Blood.Cells))
summary(pgls_elev) #p=0.18


#neutrophils ~ elevation
pgls_elev = gls(mean_value_Neutrophil.Seg.Abs/1000~Upper_elevation, data=traits_data.elev, 
               correlation = corPagel(0.5, tree.elev, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Neutrophil.Seg.Abs))
summary(pgls_elev) # p = 0.22

#lymphocytes ~ elevation
pgls_elev = gls(mean_value_Lymphocytes.Abs~Upper_elevation, data=traits_data.elev, 
               correlation = corPagel(0.5, tree.elev, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Lymphocytes.Abs))
summary(pgls_elev) #p= 0.22

#basophils ~ elevation 
pgls_elev = gls(mean_value_Basophils.Abs~Upper_elevation, data=traits_data.elev, 
               correlation = corPagel(0, tree.elev, form=~Species, fixed=TRUE), 
               method="REML", weights=~I(1/n_ind_Basophils.Abs))
summary(pgls_elev) #p= 0.84

#eosinophils ~ elevation
pgls_elev = gls(mean_value_Eosinophils.Abs~Upper_elevation, data=traits_data.elev, 
               correlation = corPagel(0.5, tree.elev, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Eosinophils.Abs))
summary(pgls_elev) #p= 0.79

#monocytes ~ elevation *** approaching significance
pgls_elev = gls(mean_value_Monocytes.Abs~Upper_elevation, data=traits_data.elev, 
               correlation = corPagel(0.5, tree.elev, form=~Species, fixed=FALSE), 
               method="REML", weights=~I(1/n_ind_Monocytes.Abs))
summary(pgls_elev) #p= 0.07, lambda=0.82

#RBC ~ elevation
pgls_elev = gls(mean_value_Red.Blood.Cells~Upper_elevation, data=traits_data.elev, 
               correlation = corPagel(0.5, tree.elev, form=~Species, fixed=FALSE),
               method="REML", weights=~I(1/n_ind_Red.Blood.Cells))
summary(pgls_elev)# *  p = 0.0059; lambda >1
...
para_types = read.csv("parasite types strepsirrhines.csv", header=T)
para_types = para_types[,2:7]
colnames(para_types)[1] = "Species"
traits_data_paratype = traits_data %>% left_join(para_types)
traits_data_paratype$prop_helm = traits_data_paratype$Helminth / traits_data_paratype$PSR
View(traits_data_paratype)
traits_data_paratype$mean_value_Eosinophils.Abs
plot(prop_helm~mean_value_Eosinophils.Abs, data = subset(traits_data_paratype, mean_value_Eosinophils.Abs<600))

mod=lm(prop_helm~mean_value_Eosinophils.Abs, data = subset(traits_data_paratype, mean_value_Eosinophils.Abs<600))
summary(mod)

plot(Helminth~log10(WOS_all), data = traits_data_paratype)

traits_data_paratype_sub =subset(traits_data_paratype, mean_value_Eosinophils.Abs<600 & !is.na(Helminth))

mod2 = lm(Helminth~log10(WOS_parasites), data = traits_data_paratype_sub)
summary(mod2)
plot(Helminth~log10(WOS_parasites), data = traits_data_paratype_sub)

traits_data_paratype_sub$helmresid = residuals(mod2)

plot(helmresid~mean_value_Eosinophils.Abs, data = traits_data_paratype_sub)
mod3 = lm(helmresid~mean_value_Eosinophils.Abs, data = traits_data_paratype_sub)
summary(mod3)


plot(Helminth~mean_value_Eosinophils.Abs, data = traits_data_paratype_sub)
mod3 = lm(Helminth~mean_value_Eosinophils.Abs, data = traits_data_paratype_sub)
summary(mod3)

#should re-do all parasite analyses with PGLS just to be sure
